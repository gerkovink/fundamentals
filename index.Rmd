...
**No need to hand in any work; but make sure that you understand the contents, code and train ot thought of the document. If not, ask Gerko or Justin.**
...
**To allow you to enjoy the holidays, there is no need to hand in any work; but make sure that you understand the contents, code and train ot thought of the document. If not, ask Gerko or Justin.**
...
1. Recreate the plots from slides 26 and 27, but now for females
...
3. See if a model with all variables included is a better fit than the model with the interactions presented in the slides. 
  - The model with all variables (excluding `Name`!) can be simply fit with
```{r eval=FALSE}
glm(Survived ~ . * ., data = titanic[, -3]) # exclude 3rd column `Name`
```
  - The `.` here implies all remaining columns in the data. `. * .` yields the full factorial model between all these columns. 

Hand in a markdown compiled `html` file for Exercise 9 to the [SurfDrive](https://surfdrive.surf.nl/files/index.php/s/yx5SsHz3haHLhqK) drop folder. Do this ***before the next lecture on Monday***.
