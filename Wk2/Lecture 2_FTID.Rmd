...
plotdata %>%
  ...
  geom_point(color = "blue") + 
  geom_point(aes(167.5, 180.8), color = "orange", size = 5) + 
  geom_segment(aes(xend = 167.5, yend = Y), color = "orange", lty = 2, alpha = .5) +
  geom_vline(xintercept = 167.5, color = "orange") + 
...
plotdata %>%
  ...
  geom_segment(aes(xend = X, yend = 180.8), color = "orange", lty = 2, alpha = .5) + 
  geom_hline(yintercept = 180.8, color = "orange") +
...
## Least squares solution
```{r echo=FALSE}
data.frame(X = rnorm(100, 167.5, 10)) %>%
