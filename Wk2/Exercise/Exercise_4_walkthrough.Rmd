...
The values in `x` have drawn from a population with mean $\mu = 3$ and standard deviation $\sigma = 7$. 
...
```{r}
lsfun2 <- function(meanest){
  apply(outer(x, meanest, "-")^2, 
        MARGIN =  2, 
        FUN = sum)
}
```
...
but now use the sample mean $\bar{x}$ in your function `lsfun()`. Let the function plot the curve and print the location where the minimum of the sum of the squares is located each time. Fix the seed to `set.seed(123)`:**

```{r}
hist(y)
lsfun <- function(meanest) apply(outer(y, meanest, "-")^2, 2, sum)
curve(lsfun, from = 1, to = 12)
```
