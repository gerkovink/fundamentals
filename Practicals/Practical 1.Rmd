...
Run the below code example and see what happens. 
...
Add the following to the above code example:
...
Two questions to evaluate your progress.
...
Create a new object `b` and assign to it `a+a`. Check if `b` is indeed `a+a`. 
...
Now square `b`, multiply the answer with $\frac{a}{b}$ and call the result `c`. Then, take the square root of $c^b$ and multiply it with $\frac{a}{b^6}$. 
...
Round the answer from the previous exercise to 3 decimals.
...
# Objects with more elements

---

In this practical we are going to play around with the different types of objects and elements in R. 

---

## Exercise 8

Make two vectors: one named `vec1` with values 1 through 6 and one named `vec2` with letters A through F and inspect them 
...
To create a vector we used `c()`, which stands for 'concatenate'. It is just a series of numbers or letters. 

---

## Exercise 9

Now extend the code with two matrices, one from `vec1` and one from `vec2`. The dimensions for both matrices are 3 rows by 2 columns.
